...
public interface QueryConstants {
    ...
    String RECOMMENDATIONS_QUERY = "SELECT+Id,LastModifiedDate,CreatedDate,Name,OwnerId,Owner.Name,TH_Account_Name__c,TH_Additional_Information__c,TH_Asset__c,TH_City__c,TH_Closure_Notes__c,TH_Description_for__c,TH_Description__c,TH_Interaction_Planned_Date__c,TH_Opportunity__c," +
            ...
            "TH_Spare__r.TH_LifeCycle_days__c,TH_Spare__r.TH_ORDER_LINE_ID__c,TH_Status__c,TH_SubType__c,TH_Type__c,TH_Selected_Services__c,TH_Part_Code__c,TH_Selected_Spares__c+FROM+TH_Interaction__c where  recordtype.name = 'Recommendation' and TH_Asset__r.Visibility_in_Sales_Portal__c+=+TRUE and TH_Asset__r.THCH_Asset_Status__c+=+'Active' and lastmodifiedDate = +";
    ...
    String COMPLAINTS_QUERY = "SELECT+Id,Owner_Role_name__c,Owners_MobilePhone__c,CaseNumber,RecordType.Name,Reason,OwnerId,owner.Name,owner.email,Status,Subject" +
            ",Description,THS_Source__c,AccountId,AssetId,ContactId,THS_Division__c,Zone__c,THS_Sub_Division__c,Origin,THS_Case_Source__c,TH_Region__c,THS_Dept_Dependency__c,THC_Country__c,THS_Asset_Status__c,Priority,CreatedDate,lastmodifieddate+from+case+where+THS_Source__c='Customer Complaint'and+THS_Case_Source__c='Customer'+and+lastmodifieddate+=";
